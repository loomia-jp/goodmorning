...
├── morning-brief/
│   ├── SCHEMA.md             # 日次 JSON スキーマ（v1.1）
│   ├── PROMPT.md             # 朝刊プロンプト（4 Subagent 並列 / 福祉独立）
│   ├── templates/
│   │   └── email-brief.html  # 朝刊 HTML テンプレート（福祉セクション含む）
│   └── data/
│       ├── README.md
│       └── YYYY/MM/DD.json   # 自動生成・自動コミット
├── topics-brief/
│   └── TOPICS_PROMPT.md      # 話のネタ便プロンプト（3 Subagent 並列 / Phase 3 で配信化）
└── .github/workflows/
...
- **Subagent 並列実行**：
  - 朝刊：4 Subagent（相場・経済 / 買取・リユース業界 / AI・テック / **福祉・介護・終活**）を Task ツールで並列起動
